...
def test_intantiate_over():
    ...
    # pre cleanup
    for executing_path in glob.glob('executing*.endomill.ipynb'):
        unlink_missing_ok(executing_path)
    ...
    assert not glob.glob('executing*.endomill.ipynb')
    for expected_output in expected_outputs:
...
def test_intantiate_over_multiprocess():
    expected_outputs = [
        'parameter=value1+ext=.endomill.ipynb',
        'parameter=value2+ext=.endomill.ipynb',
    ]
    ...
    notebook_path \
        = f'{dirname(realpath(__file__))}/assets/instantiate_over_multiprocess.ipynb'
    os.environ['NOTEBOOK_PATH'] = notebook_path
    nest_asyncio.apply()
    nb = nbformat.read(
        notebook_path,
